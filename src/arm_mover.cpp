...
std::vector<float> clamp_at_boundaries(float requested_j1, float requested_j2)
{
    ...
    if (requested_j1 < min_j1 || requested_j1 > max_j1)
    {
        ...
        ROS_WARN(node_name + ": j1 is out of bounds, valid range (%1.2f,%1.2f), clamping to: %1.2f", min_j1, max_j1, clamped_j1);
    }
    ...
    if (requested_j2 < min_j2 || requested_j2 > max_j2)
    {
        ...
        ROS_WARN(node_name + ": j2 is out of bounds, valid range (%1.2f,%1.2f), clamping to: %1.2f", min_j2, max_j2, clamped_j2);
    }
...
{
    std::string node_name = ros::this_node::getName();
    ROS_INFO(node_name + ": GoToPositionRequest received - j1:%1.2f, j2:%1.2f", (float)req.joint_1, (float)req.joint_2);
    ...
    res.msg_feedback = node_name + ": Joint angles set - j1: " + std::to_string(joints_angles[0]) + " , j2: " + std::to_string(joints_angles[1]);
...
int main(int argc, char **argv)
{
    ...
    ROS_INFO(node_name + ": Ready to send joint commands");
